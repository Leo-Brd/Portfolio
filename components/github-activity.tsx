    ...
    <div className="p-6 mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.slice(0, 6).map((event) => (
          ...
          >
            <Card className="h-full hover:shadow-lg dark:shadow-white/10 transition-shadow duration-300">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span className="text-lg font-semibold">
                    {event.type.replaceAll(/([A-Z])/g, " $1").trim()}
                  </span>
                  <ArrowRight className="w-5 h-5 text-primary hover:text-black dark:hover:text-primary-foreground" />
                </CardTitle>
              </CardHeader>
              <CardContent>
                ...
                {event.payload?.commits?.map((commit) => (
                  <p key={commit.sha} className="text-sm mt-2">
